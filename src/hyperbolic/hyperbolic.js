...
export function movePointToOrigin(Z, A) {
  return complexQuotient(
    complexDifference(Z, A),
    complexDifference([1, 0], complexProduct(complexConjugate(A), Z)),
  );
}
...
export function movePointFromOrigin(Z, A) {
  return complexQuotient(
    complexSum(Z, A),
    complexSum([1, 0], complexProduct(complexConjugate(A), Z)),
  );
}
...
export function hyperCircle(board, A, r, style = circleStyle) {
  const center = () =>
    hyperCircleCenter(getXY(A), typeof r === "function" ? r() : r);
  const radius = () =>
    hyperCircleRadius(getXY(A), typeof r === "function" ? r() : r);
...
export function clineCenter(A, B) {
  ...
  const D = complexDifference(
    ...
    complexProduct(complexConjugate(A), B),
  );
...
export function hyperlineEndPoint(A, B) {
  const Bp = movePointToOrigin(B, A);
  const Ep = pointScale(Bp, -1 / pointDist(Bp));
...
export function hyperReflect(A, B, P) {
  ...
  const D = complexSum(
    complexProduct(Z2, complexConjugate(P)),
    complexConjugate(Z1),
  );
  return complexQuotient(N, D);
}
...
export function hyperAngle(A, B, C) {
  ...
  return Math.acos(
    (Math.cosh(a) * Math.cosh(c) - Math.cosh(b)) /
      (Math.sinh(a) * Math.sinh(c)),
  );
}
...
export function hyperInradius(A, B, C) {
  ...
  return Math.atanh(
    Math.sqrt(
      (Math.sinh(s - a) * Math.sinh(s - b) * Math.sinh(s - c)) / Math.sinh(s),
    ),
  );
}
